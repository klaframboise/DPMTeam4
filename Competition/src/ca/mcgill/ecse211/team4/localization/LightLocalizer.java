...
public class LightLocalizer {
	 ...
	 * Using this method will cause the robot to move until the color sensor crosses
	 * the next grid line intersection in the positive x and y axis. Be careful when
	 * using this near forbidden areas.
	 * @param startingCorner [1,4].
	...
	public boolean localize(int startingCorner) {
		...
		case 4:
			...
			Robot.getOdo().setY(11 * Robot.GRID_SIZE + dx);
...
//			}
		...
		/* Only adjust odometer if this is not the first localization because starting corner has
		 * to be taken into consideration while localizing for the first time
		 */
	...
	private void adjust(char axis) {
		...
		switch(axis) {
		case 'y': Robot.getNav().turnTo(Math.PI/2.0); break;	//y-axis is in positive y direction
		case 'x': Robot.getNav().turnTo(0); break;		//x-axis is in positive x direction
		default: return;
		}
