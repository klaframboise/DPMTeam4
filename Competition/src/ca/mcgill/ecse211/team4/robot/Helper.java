...
/**
 * This class provides helpful methods that can be used throughout the project. All the methods in
 * this class are static. This is to avoid code duplication and consolidate methods that are used
 * often by different classes.
 * 
...
public abstract class Helper {
  ...
  /**
   * This method converts a distance to a rotation amount to be requested to the motors. This method
   * was provided during lab 2 and is used as-is.
   * 
   * @param wheelRadius radius of the robot's wheel.
  ...
  public static int getFilteredDistance(SampleProvider us, float[] usData) {
    ...
    return (int) ((usData[(usData.length / 2) - 1] + usData[usData.length / 2]) / 2.0 * 100.0); // return
                                                                                                // median
  }
   ...
   * @param lowerLeftX
   * @param lowerLeftY
   * @return a bidimensional array containing the coordinates of the four corners of a rectangular
   *         area. The array is structured as follows: coords[i][j], i cycles corners from upper
   *         right, to lower right, to lower left, to upper left. j = 0 is the x coordinate, j = 1,
   *         the y.
   */
  public static int[][] convertToFourCorners(int upperRightX, int upperRightY, int lowerLeftX,
      int lowerLeftY) {
